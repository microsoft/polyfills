...
- **`Polyfills - CD Build`** uses `azure-pipelines-build.yml`. A `main` build checks every non-private npm workspace for its `${name}_v${version}` tag. If any tag is missing, the `BuildArtifacts` stage installs dependencies, builds the workspaces, packs the corresponding npm tarballs, and publishes both the packages and release metadata.
...
The build pipeline's `validationMode` can rebuild already-tagged package versions. Run the CD pipeline manually against that build with its matching `validationMode` to validate artifacts without creating tags, releases, or npm publications.

...
5. The Azure CD pipeline checks for GitHub Releases that do not have `deployed/<tag>` marker tags. For each undeployed release, it downloads tarball assets using the Azure GitHub service connection named `fast`, publishes the packages to npm, and pushes `deployed/<tag>` marker tags on success.

Release environment assumptions:

- The Azure GitHub service connection is named `fast` and can read release assets from `microsoft/polyfills`.

...
export function parseTypeSyntax(rawInput: string): TypeSyntaxParseResult {
  ...
  const parsed = parseComponentValues(raw);
  ...
  const values = trimTrivia(parsed.values);
  let syntax: string;
  let universal = false;
  ...
  if (
    ...
    syntax = serializeComponentValues(trimTrivia(values[0].value)).trim();
    if (!syntax) {
    ...
    syntax = serializeComponentValues(values).trim();
...
export function checkType(value: string, type: TypeSyntax): TypeCheckResult {
  if (type.universal) {
    return { status: "valid" };
  }
  ...
  try {
    const match = lexer.match(type.syntax, value);
    if (!match.error) {
      return { status: "valid" };
    }

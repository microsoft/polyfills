...
function parseResolvedValue(value: ResolvedValue): Resolution {
  const parsed = parseComponentValues(value.value);
  if (parsed.errors.length > 0) {
    return failure(
      "invalid-function-call",
      "A substituted value contains invalid CSS syntax.",
    );
  }

  return success(parsed.values, value.dynamic);
}
...
function resolveExpression(
  ...
  if (parsed.errors.length > 0) {
    return failure(
      "invalid-function-call",
      "The value contains invalid CSS syntax.",
    );
  }

  return resolveComponents(parsed.values, context);
}
...
function resolveBinding(
  ...
  if (binding.resolved) {
    return parseResolvedValue(binding.resolved);
  }
  ...
  if (cssWideKeywords.has(source.trim().toLowerCase())) {
    return failure(
      "unsupported-css-wide-keyword",
      `The "${binding.name}" binding uses the CSS-wide keyword "${source.trim()}", which requires runtime scope semantics.`,
      binding.frame.definition.name,
    );
  }
  ...
  const value = {
    ...
    value: serializeComponentValues(resolved.nodes),
  };
  ...
  return success(resolved.nodes, resolved.dynamic, resolved.changed);
}
...
function resolveVar(
  ...
  const split = splitOnFirstComma(node.value);
  const nameValues = trimTrivia(split.before);
  const name = tokenIdentValue(nameValues[0]);

  if (!name?.startsWith("--") || nameValues.length !== 1) {
    return failure(
      "invalid-function-call",
  ...
  if (!binding) {
    const nested = resolveComponents(node.value, context);
    if (!nested.ok) {
      return nested;
    }
    node.value = nested.nodes;
    return success([node], true, nested.changed);
  }
  ...
  if (resolved.dynamic && split.after !== undefined) {
    return failure(
...
function evaluateCustomFunction(
  ...
  const parsedArguments = parseFunctionArguments(node);
  ...
  const resultValue = serializeComponentValues(resolved.nodes);
  if (definition.returnType) {
    const checked = checkType(resultValue, definition.returnType);
    if (checked.status === "indeterminate") {
      return failure(
...
function resolveFunctionNode(
  ...
  if (lowerName === "var") {
    return resolveVar(node, context);
  }

  if (name.startsWith("--")) {
    if (context.state.registry.has(name)) {
      return evaluateCustomFunction(node, context);
    }
  ...
  const runtimeDependent =
    lowerName === "attr" || lowerName === "env" || lowerName === "if";

  return success([node], nested.dynamic || runtimeDependent, nested.changed);
}
...
function resolveComponents(
  ...
  for (const value of values) {
    ...
    if (isFunctionNode(value)) {
      resolved = resolveFunctionNode(value, context);
    } else if (isSimpleBlockNode(value)) {
      const nested = resolveComponents(value.value, context);
      if (!nested.ok) {
        return nested;
      }
      value.value = nested.nodes;
      resolved = success([value], nested.dynamic, nested.changed);
    } else {

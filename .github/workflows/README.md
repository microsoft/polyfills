...
Nightly publishing is split into two coordinated jobs. GitHub Releases are the source of truth, and `deployed/<tag>` git marker tags track which releases have already been published.
...
Tags that already have a `deployed/<tag>` marker skip this network call entirely; only undeployed tags are checked against the release API.
